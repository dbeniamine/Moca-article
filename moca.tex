...
\begin{abstract}
    ...
    patterns aiming at locality and linearity.

    Most efforts on memory performances optimization at software level relies
    on adaptive tools which move data or thread on CPUs at the runtime to
    reduce communication times. Such tools are often efficient but cannot
    modify an application's memory access pattern thus they are not efficient
    on application written without considering memory behavior. The developer
    still need to consider this behavior.
    ...
    developer understanding and therefore optimizing its application memory
    behavior resulting in significant performance gain.

    In this study we present \Moca an efficient memory trace collection
    system, based on a Linux kernel module, that provide a complete overview
    of an applications memory behaviour over the time. We evaluate \Moca
    overhead and we compare it to existing tools in terms of performances and
    trace details.
