...
\begin{abstract}
    ...
    are a common bottleneck. Therefore optimizing the way an application
    access the memory can provide significant performance improvements.

    Most efforts on memory accesses optimization consists on adaptive tools which
    move data or thread on the CPUs at the runtime to reduce and optimize
    communications.
    ...
    provide an overview of the memory accesses.

    In this study we present \Moca an efficient memory trace collection system
    that provide a complete overview of an applications memory behaviour over
    the time. We evaluate \Moca overhead and we compare it to existing tools
    in terms of performances and trace precision.
