...
\DB{Add ref review1 HPDC ?\\
    Bae et al.: x86 only, based on fpf, compute metrics on mem accesses to generate
    profile, then use to compute page mapping
    \url{http://dl.acm.org/citation.cfm?id=2287114}\\
    Geiger (Jones et al.): base on pf, send data to VMM about memory for
    online use: \url{http://dl.acm.org/citation.cfm?id=1168861}\\
    Antfarm (Jones et al.): map processes to addressespace on VMM, online usage
    \url{https://www.usenix.org/legacy/event/usenix06/tech/full\_papers/jones/jones\_html/}}
