\begin{tikzpicture}[node distance = 2cm,scale=0.8]
    ...
    \node[entity,text width=6em,pf,below right=5em and 4.5em of M] (T) {Tasks\\(user/kernel\\thread/process)};

    \node [dataL, below left=2em and .5em of T]    (pgt)      {Page table (kernel)};
    ...
    % Page faults
    \path[pfA] (T.north)   edge[out=80,in=0] node[pos=.6,left] {triggers} (PfH.east);
    ...
    \path[pfA] (PfH.south)  edge[out=270,in=100] node[pos=.25,left=.5em] {read\\write} (pgt.north);
