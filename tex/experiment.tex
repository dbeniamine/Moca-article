...
For every machine, hyper threading was disabled during the experiments and we
deployed the same \emph{Debian} \emph{Wheezy} environment running a \texttt{Linux 3.2.41-2}.
...
\label{sec:expe-ovh}
\DB{New experiments:\\
        + Edel: Moca / MocaPin / Pin (tabarnac) / Mitos
        \\
        + Idfreeze: Moca / MocaPin (needed ?) / MemProf
}
...
\DB{See new figure, maybe log scale ?
        \\ + Randomize runs (waiting 2015-01-07)
        \\ + Bigger FalsePfMap
        \\ + Figure log scale
}
