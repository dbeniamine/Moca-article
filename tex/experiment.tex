...
which relies on AMD IBS. Every tool is evaluate with its default parameters,
    ...
    \DB{Mitos et MemProf => default + tuné pour nombre d'evenements
    comparable. Conclure Moca seul designé et efficace pour cartographie}
...
\DB{Rewritte text:\\
    + Warning log scale !\\
    pages:\\
    + Moca and Tabarnac capture every pages \\
    + MocaPin have a bit more pages due to pin stuff, but not much => trace
    still reliable\\
    + Mitos trace are way insufficient\\
    + Mitostun ? \\
    Addr:\\
    + Moca Better then every tools\\
}
...
\DB{Rewritte text:\\
    + Warning log scale !\\
    + Mitos almost no overhead => but previous expe show traces are not
    precise\\
    + Moca usually significantly bettr then tabarnac except ft dc and mg\\
    + Pin additional overhead is negligible.
    + Explain mg\\
    cncl:\\
    + Moca overhead is more precise than the most precise tools existing and
    faster then them => yeay !
}
