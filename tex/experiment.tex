...
execution time and precision.\GH{Comment changer l'intervalle de logging peut-il avoir un impact sur le nombre d'évènements capturés ? Autre question un peu en
...
\TABARNAC result from runs on \Edel machines, while \MemProf values result from runs on
\Stremi.
...
collected by \Moca\GH{Je ne vois pas ça sur la figure, erreur ?}.
...
memory accesses, \CG is memory intensive,
...
described in \tbl{NPB}.\GH{Ce n'est pas ce que je vois dans la table : UA semble comme les autres et le chiffre pour DC, 146Gib me paraît énorme, erreur ?}
...
thread scheduling issue\GH{Ca, pour moi, ça sort du chapeau. Tu as un argument ? Moi je dirais plutôt qu'il balai largement son espace mémoire (même petit)
durant toute son exécution} rather than from the size of the benchmark. By looking at our
