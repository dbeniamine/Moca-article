...
distribute every file needed to reproduce our experiments at three different
levels:
\DB{Add links}
\begin{enumerate}
    \item \textbf{Statistic analysis:} To reproduce our statistic analysis we provide
        the filtered results from the experiment along with the
        \texttt{R-markdown} scripts that generated the plots presented in this
        article\footnote{\url{link TODO}}.
    \item \textbf{Raw analysis:} We also distribute the full raw
        traces\footnote{\url{link TODO}} generated by
        our experiments with the scripts used to extract the filtered traces
        and the analysis scripts.
    \item \textbf{Experiment reproducibility:} Finally we provide a git
        repository\footnote{\url{https://github.com/dbeniamine/hpdc_expe}}
        including our deployment environment, dependencies to each tools and
        every files needed for the experiments and instruction on how to
        reproduce the experiments with and without access to grid5000.
\end{enumerate}
