...
For every experiment, deployed the same \emph{Debian} \emph{Jessie}
...
\fig{fig:pages} confirms that, as stated previously, only \Moca and \TABARNAC are
...
\fig{fig:addr} shows that in terms of number of unique addresses captured,
...
These results proves that \Moca is the only existing tool able to provide a
...
\DB{\MitosTun and \MemProf / Explain \MemProf integration in the plot}
