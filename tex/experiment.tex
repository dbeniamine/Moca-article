...
\begin{table}[htb]
    ...
    \begin{tabular}{p{1.5cm}lcp{3.3cm}}
        ...
        & \SP & \si{?}{Mib}& Scalar Penta-diagonal \\
        & \LU & \si{?}{Mib}& Lower-Upper Gauss-Seidel \\
        \midrule
        \multirow{2}{1.5cm}{Unstructured}
        & \UA & \si{?}{Mib}& Unstructured Adaptive mesh \\
        & \DC & \si{?}{Mib}& Data Cube \\
        \midrule
        CPU bound & \EP & \si{?}{Mib}& Embarrassingly parallel \\
        \bottomrule
    \end{tabular}
...
\DB{Finish valgrind runs}

In the following sections, all the tools are
evaluated on each of the 10 \NPB~\cite{Jin1999}, these benchmarks are
presented in \tbl{tab:NPB}, more precise description are available online\footnote{\url{http://www.nas.nasa.gov/publications/npb.html}}.
...
\DB{Rewrite with ref to \tbl{tbl:NPB}}
...
it is made of varied application profiles as we can see in \tbl{tbl:NPB}.
...
parallel I/O and data movement}, its
