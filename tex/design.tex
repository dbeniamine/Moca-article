...
To make the trace more readable, \Moca also retrieve data structure
names, addresses and size using a Pin~\cite{Luk05Pin}
instrumentation. When enabled, this instrumentation reads static data
...
and names malloced structures according to their call path. This
instrumentation adds a (small) constant overhead when Pin opens a binary and on
every allocations. Moreover, while Pin generate some additional processes that
could interfere with the analysis, they can be easily identified by their name
and are ignored by \Moca.  The experiments presented in
section~\ref{sec:expe-ovh} shows that this overhead is compensated by the fact
that the noise induced by Pin is not memory intensive, thus it release a part
of the pressure on \Moca reducing its overhead.
...
of the time, each chunk embed two timestamp to delimit its beginning and end.
...
internal id of the task which triggered it. A set of access sharing the same
timestamps and task id correspond to a chunk. The order of accesses inside a
chunk is preserved.
