...
Each time a page fault occurs, it is traped by the \Moca handler that uses
...
\GH{Je ne comprend rien à ce paragraphe\ldots}
In the first
case, \Moca \emph{fixes} it by setting the present flag on the \texttt{Page
Table Entry} and by marking the according hashmap entry as \emph{BAD} so we only need a read
...
for each CPU the last address that faulted, and does not clear this
address.
