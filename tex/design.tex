...
instrumentation. When enabled \Moca runs the application twice with virtual
...
every allocations.
Running separately \Moca and Pin prevents Pin to interfere
with the trace quality. Furthermore the instrumentation only adds a small overhead when
opening a binary file and during the initialization thus the overhead is quite
similar to \Moca overhead.
